...
def matriz_de_errores(n, y, l):
    ...
    def aproximacion(a1, a3): return a1*l[1]+a3*l[3]
    ...
    for i in range(0, n):
        for j in range(0, n):
            ...
            mError[i][j] = np.linalg.norm(
                y-aproximacion(alpha1[i], alpha3[j]))**2
...
def ejer_3():
    ...
    print(
        f'error cuadratico con una aproximacion de 4: {np.linalg.norm(y-aproximacion1)**2}')
    print(
        f'error cuadratico con una aproximacion de 6: {np.linalg.norm(y-aproximacion2)**2}')
    print(a)
