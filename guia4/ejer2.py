import numpy as np
from utils.Signals import senoidal, cuadrada
from guia4.fft import my_fft, epsilon


def ejer2(inciso=0):
    # definimos el producto interno
    def producto_interno(a, b): return np.dot(a, np.conj(b))
    ...
    # seniales
    sn = ['a', 'b', 'c']
    s = np.array([senoidal(t0, t1, fm, 2)[1], cuadrada(
        t0, t1, fm, 2)[1], senoidal(t0, t1, fm, 4)[1]])
    if inciso:
        sn = ['a', 'c']
        s = np.array([senoidal(t0, t1, fm, 2)[1],
                     senoidal(t0, t1, fm, 3.5)[1]])
